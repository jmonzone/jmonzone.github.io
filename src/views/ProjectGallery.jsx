...
export default function ProjectGallery({
  ...
  if (currentTag == tags.All) projectsToShow = Object.keys(projects);
    ...
    Object.keys(projects).forEach(project => {
      if (projects[project].tags.includes(currentTag))
  ...
  const backToGallery = (
    <div className={`projects-navigation-tag`} onClick={() => setProject(null)}>
      Back To Projects
    </div>
  );
  ...
  const navigation = currentProject
    ...
    : Object.keys(tags).map(tag => (
        <div
          className={`projects-navigation-tag ${
            tag == currentTag ? 'active' : 'inactive'
          }`}
          key={tag}
          onClick={() => onNavigationTagClicked(tag)}
        >
          {tags[tag]}
        </div>
      ));
  ...
  return (
    <div className={`projects`}>
      <div className="projects-navigation">{navigation}</div>
      <ProjectSpotlight project={currentProject} />
      <div
        className={`projects-gallery ${currentProject ? 'hidden' : 'active'}`}
      >
        <div className="projects-gallery-header">
          {tags[currentTag]} Projects
        </div>
